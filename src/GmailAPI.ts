...
function getLabelName(id: string, labels: Array<Array<string>>) {
	for (let i = 0; i < labels.length; i++)
		if (id == labels[i][1]) return labels[i][0];
	return "";
}
...
async function getAttachments(
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (!filename || !attach_id) {
			console.debug(
				msgId,
				`Part ${i} has no filename or attachmentId, skipping...`,
			);
			continue;
		}
...
function flatten_parts(mbObj: MailboxObject, parts: MessagePart[]) {
	if (
		...
		parts[1].mimeType == "text/html"
	) {
		assertPresent(
			parts[0].body,
			"MessagePart had text/plain MIME type but no body!",
		);
		assertPresent(
			parts[1].body,
			"MessagePart had text/html MIME type but no body!",
		);
		...
		parts.forEach((part) => {
			if (
				...
				assertPresent(
					part.parts,
					"MessagePart had mixed MIME type but no sub-parts!",
				);
...
async function saveMail(settings: ObsGMailSettings, id: string) {
	...
	const title_candidates: PayloadHeaders =
		(res.data.messages || [])[0].payload?.headers || [];
	const labels = getLabelIDs(res.data).map((labelID: string) =>
		getLabelName(labelID, settings.labels),
	);
	...
	fields.set(
		"${Labels}",
		labels
			.map((label: string) => note.label_format.replace(/\{\}/, label))
			.join(", "),
	);
	...
	const body = await processBody(
		[mailboxObject.raw_mtxt, mailboxObject.raw_mhtml],
		note.body_format,
	);
...
async function fetchMails(settings: ObsGMailSettings) {
	...
	if (threads.length > amount)
		new Notice(
			`Gmail: There are ${threads.length - amount} mails not fetched.`,
		);
	else new Notice(`Gmail: Your inbox is up to date`);
}
