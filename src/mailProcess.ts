...
async function getBody(bodys: Array<MessagePartBody | null>, format: string) {
	...
	if (format == "htmlmd") body = await processHTMLBody(normalizedBodys[1]);
	else if (format == "text") body = await processPTBody(normalizedBodys[0]);
	else body = await processRawBody(normalizedBodys[1]);
	if (body == "")
...
export async function processBody(
	bodys: Array<MessagePartBody | null>,
	format: string,
) {
